...
const init = async (mod: Spotfire.Mod) => {
    ...
    // Show fade-in/-out cogwheel and infoIcon on visualization mouseover
    modContainer.on("mouseover", () => {
            if (context.isEditing) cogwheel.style("opacity", "1");
    ...
    async function render(
        ...
        if (!somethingChanged && prevHash == hash) {
            ...
            if (context.isEditing) createSettingsPopout();
        }
        prevHash = hash;
